#!/bin/bash
#================================================================
# SLURM Job Script: LiTS Stage 2 Training (no W&B)
...
echo "========================================"
echo "LiTS Stage 2 Training - U-Net + ResNet50 (no W&B)"
echo "Date: $(date)"
echo "Node: $(hostname)"
...
if [ -n "${RESUME}" ]; then
    ...
    $PYTHON src/train_stage2.py --config "${CONFIG}" --resume "${RESUME}"
else
    $PYTHON src/train_stage2.py --config "${CONFIG}"
fi
